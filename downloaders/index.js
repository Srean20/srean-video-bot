...
async function getFormats(url) {
    ...
    console.log(`[getFormats] ${url.substring(0, 40)}... → Returning MP4/MP3/Full options`);
...
async function downloadVideo(url, selector) {
    ...
    if (selector === 'mp3') {
        args = [
            ...
            url,
            '-x', '--audio-format', 'mp3',
            '--no-playlist', '--socket-timeout', '60', '--no-warnings',
            '-o', outputTemplate,
        ];
    } else {
    ...
    console.log(`[download] cmd: ${cmd} ${args.join(' ')}`);
    ...
    let ext = path.extname(filePath.name).toLowerCase();

    // MP4/MP3 check
    if (ext !== '.mp4' && ext !== '.mp3') {
        fs.rmSync(tmpDir, { recursive: true, force: true });
        throw new Error('File Not Support');
    }
    ...
    if (size > MAX_BYTES) {
        fs.rmSync(tmpDir, { recursive: true, force: true });
        throw new Error(
            `File is too large (${(size / 1024 / 1024).toFixed(1)} MB). ` +
            `Max is ${MAX_FILE_SIZE_MB} MB. Please choose a lower quality.`
        );
    }

    const title = path.basename(filePath.name, ext);
